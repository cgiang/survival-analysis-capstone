...
```{r}
# look again
IPO %>% select(c("IPODate", "YearFounded", "DateFounded","exactDiffernce"))
```

```{r}
# subset of the original data set without TVC or anything after Fiscal_year
IPO_sub <- select (IPO, IPODate, YearFounded, DateFounded, Symbol, DaysBetterThanSP, daysProfit, daysProfitGrouped, exactDiffernce, Name, LastSale, MarketCap, Sector, Industry, CEOTakeOver, CEOAge, PresidentAge, Revenue, netIncome,  lastFiscalYearGrowth, employees)
```
...
```{r}
data<-select(IPO_sub,DaysBetterThanSP,daysProfit,exactDiffernce,LastSale,MarketCap,CEOAge,PresidentAge,Revenue,netIncome,lastFiscalYearGrowth,employees)
summary(data)
```
...
```{r,eval=FALSE}
m1 = coxph( Surv( exactDiffernce) ~ DaysBetterThanSP , data=IPO_sub )
plot( cox.zph(m1) )

m2 = coxph( Surv( exactDiffernce) ~ daysProfit , data=IPO_sub )
plot( cox.zph(m2) )

m3 = coxph( Surv( exactDiffernce) ~ Revenue , data=IPO_sub )
plot( cox.zph(m3) )

m4 = coxph( Surv( exactDiffernce) ~ netIncome , data=IPO_sub )
plot( cox.zph(m4) )

m5 = coxph( Surv( exactDiffernce) ~ CEOAge , data=IPO_sub )
plot( cox.zph(m5) )

m6 = coxph( Surv( exactDiffernce) ~ PresidentAge , data=IPO_sub )
plot( cox.zph(m6) )

m7 = coxph( Surv( exactDiffernce) ~ lastFiscalYearGrowth , data=IPO_sub )
plot( cox.zph(m7) )

m8 = coxph( Surv( exactDiffernce) ~ employees , data=IPO_sub )
plot( cox.zph(m8) )
```

```{r}
library(ggplot2)
ggplot(data=IPO_sub,aes(x=DaysBetterThanSP,y=exactDiffernce))+geom_point()+geom_smooth()
ggplot(data=IPO_sub,aes(x=DaysBetterThanSP,y=exactDiffernce))+geom_point()+geom_smooth()
```

```{r}
ggplot(data=IPO_sub,aes(x=DaysBetterThanSP,y=exactDiffernce,color=factor(daysProfitGrouped)))+geom_point()+geom_smooth()

# Alternatively
ggplot(IPO_sub,aes(x=DaysBetterThanSP,y=exactDiffernce)) +
    geom_boxplot() +
