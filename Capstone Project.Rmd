...
IPO_sub <- select (IPO, DaysToIPO, YearsToIPO, DaysBetterThanSP, daysProfit, daysProfitGrouped, LastSale, MarketCap, Sector, Industry, CEOTakeOver, CEOAge, PresidentAge, Revenue, netIncome, lastFiscalYearGrowth, employees)
```
...
```{r, message=FALSE}
data<-select(IPO_sub,-Sector,-Industry,-daysProfitGrouped)
library(pcalg)
...
```{r}
md3=survreg(Surv(DaysToIPO)~daysProfit+Revenue+netIncome+LastSale,data=IPO_sub,dist="exponential")
summary(md3)
```

```{r}
md4=survreg(Surv(DaysToIPO)~daysProfit+Revenue+netIncome+LastSale,data=IPO_sub,dist="weibull")
summary(md4)
```

```{r}
md5=survreg(Surv(DaysToIPO)~daysProfit+Revenue+netIncome+LastSale,data=IPO_sub,dist="lognormal")
summary(md5)
```
